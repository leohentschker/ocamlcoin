...
let rec verify_tree (t : MT.mtree) (n : int) : bool =
  if n = 0 then true
  else
    let tlist = MT.children t in
    let slist = sublist tlist 0 (n - 1) in
    let tn = List.nth tlist n in
    verify_transaction tn (ref (build_tree slist)) && (verify_tree t (n - 1))

let verify (t : MT.mtree) = verify_tree t (List.length (children t) - 1)
