...
let event_to_json (e : network_event) : Y.Basic.json =
  match e with
  | PingDiscovery -> print_endline "PING"; make_event_json c_PING_DISCOVERY_TYPE (`Bool true)
...
let json_to_event (json : Y.Basic.json) : network_event =
  ...
  print_endline "START JSON TO EVENT";
  let event_type = json |> member c_TRANSACTION_TYPE_KEY |> to_string in
  print_endline "EXTRACTING JSON DATA FOR EVENT";
  ...
  if event_type = c_NEW_TRANSACTION_TYPE then
    ...
    let _ = print_endline "BROADCAST NODE TO JSON" in
    ...
    let _ = print_endline "BROADCAST TRANSACTION 66" in 
    match json_data with
    | `List json_list ->
